...
export const CityTrack: React.FC<CityTrackProps> = ({
  ...
  const isSolo = players.length === 1 || gameMode === 'solo_police' || gameMode === 'solo_challenge';
    ...
    <div id="city-track-container" className="bg-slate-900/90 border border-slate-700/80 rounded-2xl p-4 shadow-xl text-white">
          ...
          <div>
            <h3 className="font-bold text-base text-slate-100 flex items-center gap-2">
              ...
              <span className="text-xs font-normal text-slate-400">
                {isSolo ? '(Старт з 0 до порту 25)' : '(26 клітинок до порту)'}
              </span>
            </h3>
            <p className="text-xs text-slate-400">
              {isSolo
                ? 'Поліція стартує на клітинці 3 і рухається щораунду. Не дозвольте їй наздогнати вас!'
        ...
        <div className="flex items-center gap-2">
          ...
          {isBehindPolice ? (
            <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-red-950/80 border border-red-500/40 text-red-300 text-xs font-semibold animate-pulse">
              ...
              <span>{isSolo ? 'Поліція попереду!' : 'Ви позаду (-3 ПО)'}</span>
            </div>
          ) : (
      ...
      <div className="relative overflow-x-auto pb-4 pt-2">
        <div className="flex items-center gap-1 min-w-[760px]">
          {(() => {
            const minSpace = isSolo ? 0 : Math.min(policePosition, -players.length, -2);
            const totalSpaces = Array.from(
              { length: CITY_TRACK_LENGTH - minSpace },
              (_, i) => minSpace + i
            );

            return totalSpaces.map(space => {
              const isCheckpoint = space >= 0 && !!CHECKPOINTS[space];
              const checkpointInfo = space >= 0 ? CHECKPOINTS[space] : null;
              ...
              return (
                <div
                  ...
                  className={`relative flex-1 min-w-[30px] h-20 rounded-xl border flex flex-col justify-between p-1 transition-all ${
                    space < 0
                      ...
                      : space === 0
                      ? 'bg-blue-950/50 border-blue-400/90 text-blue-200 font-extrabold ring-1 ring-blue-400/40'
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[10px] font-bold">
                    {space === 0 ? (
                      <span className="text-[9px] text-blue-300 font-black tracking-wide w-full text-center py-0.5 bg-blue-900/40 rounded">
                        СТАРТ
                      </span>
                    ) : (
                      <>
